...
echo "sudo apt-get install --reinstall libxcb-xinerama0"
echo "export LD_LIBRARY_PATH=$PWD/lib" >> glogg.sh
echo "$PWD/glogg \$*" >> glogg.sh
if [ ! -z "$gloggPath" ]
then
    sudo cp $PWD/glogg.sh $gloggPath
else
    sudo cp $PWD/glogg.sh /usr/bin/glogg
fi
