...
template <typename Driver>
struct ObservedFile {
  ObservedFile(const std::string& file_name, std::shared_ptr<void> callback,
...
class ObservedFileList {
 public:
  ...
  ObservedFile<Driver>* searchByFileOrSymlinkWd(
      typename Driver::FileId file_id, typename Driver::SymlinkId symlink_id);
  ObservedFile<Driver>* searchByDirWdAndName(typename Driver::DirId id,
                                             const char* name);
  std::vector<ObservedFile<Driver>*> searchByDirWd(typename Driver::DirId id);
  ...
  std::shared_ptr<ObservedDir<Driver>> watchedDirectory(
  ...
  std::shared_ptr<ObservedDir<Driver>> watchedDirectoryForFile(
      const std::string& file_name);
  std::shared_ptr<ObservedDir<Driver>> addWatchedDirectoryForFile(
      const std::string& file_name,
      std::function<void(ObservedDir<Driver>*)> remove_notification);
  ...
  class iterator
      : std::iterator<std::input_iterator_tag, ObservedFile<Driver>> {
   public:
    iterator(Container* list, const typename Container::iterator& iter) {
      list_ = list;
      pos_ = iter;
    }
    ...
    typename Container::iterator operator*() { return pos_; }
  ...
  iterator<std::list<ObservedFile<Driver>>> begin() {
    return iterator<std::list<ObservedFile<Driver>>>(&observed_files_,
                                                     observed_files_.begin());
  }
...
ObservedFile<Driver>* ObservedFileList<Driver>::searchByFileOrSymlinkWd(
    typename Driver::FileId file_id, typename Driver::SymlinkId symlink_id) {
  auto result = find_if(
      observed_files_.begin(), observed_files_.end(),
      [file_id, symlink_id](ObservedFile<Driver> file) -> bool {
        return (file_id == file.file_id_) || (symlink_id == file.symlink_id_);
      });
...
ObservedFile<Driver>* ObservedFileList<Driver>::searchByDirWdAndName(
  ...
  auto dir =
      find_if(observed_dirs_.begin(), observed_dirs_.end(),
              [id](std::pair<std::string, std::weak_ptr<ObservedDir<Driver>>> d)
                  -> bool {
...
ObservedFile<Driver>* ObservedFileList<Driver>::addNewObservedFile(
  ...
  auto new_file =
      observed_files_.insert(std::begin(observed_files_), new_observed);
...
template <typename Driver>
std::shared_ptr<ObservedDir<Driver>>
ObservedFileList<Driver>::addWatchedDirectory(
    const std::string& dir_name,
    std::function<void(ObservedDir<Driver>*)> remove_notification) {
...
template <typename Driver>
std::shared_ptr<ObservedDir<Driver>>
ObservedFileList<Driver>::watchedDirectoryForFile(
    const std::string& file_name) {
...
template <typename Driver>
std::shared_ptr<ObservedDir<Driver>>
ObservedFileList<Driver>::addWatchedDirectoryForFile(
    const std::string& file_name,
    std::function<void(ObservedDir<Driver>*)> remove_notification) {
  return addWatchedDirectory(directory_path(file_name), remove_notification);
}
...
void ObservedFileList<Driver>::cleanRefsToExpiredDirs() {
  for (auto it = std::begin(observed_dirs_); it != std::end(observed_dirs_);) {
