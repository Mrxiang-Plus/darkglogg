...
#ifndef PERSISTENTPATTERN_H
#define PERSISTENTPATTERN_H
...
class PersistentPattern {
 ...
 private:
  ...
  PersistentPattern();
  PersistentPattern(const PersistentPattern&);
  ~PersistentPattern();
  ...
  friend PersistentPattern& GetPersistentPattern();
};

PersistentPattern& GetPersistentPattern();

// Global function used to get a reference to an object
// from the PersistentPattern store
template <typename T>
std::shared_ptr<T> PatternPersistent(const char* name) {
  std::shared_ptr<Persistable> p =
      GetPersistentPattern().getPersistable(QString(name));
  return std::dynamic_pointer_cast<T>(p);
}

template <typename T>
std::shared_ptr<T> PatternPersistentCopy(const char* name) {
  std::shared_ptr<Persistable> p =
      GetPersistentPattern().getPersistable(QString(name));
