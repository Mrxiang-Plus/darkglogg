...
KQueueWatchTowerDriver::KQueueWatchTowerDriver() : kqueue_fd_(kqueue()) {
  ...
  if (kevent(kqueue_fd_, &changelist, 1, NULL, 0, &no_timeout) != -1)
    LOG(logDEBUG) << "KQueueWatchTowerDriver::KQueueWatchTowerDriver breaking "
                     "pipe monitored";
  else
    LOG(logERROR) << "KQueueWatchTowerDriver::KQueueWatchTowerDriver cannot "
                     "add breaking pipe";
}
...
KQueueWatchTowerDriver::FileId KQueueWatchTowerDriver::addFile(
  ...
  if (fd != -1) {
    LOG(logDEBUG) << "KQueueWatchTowerDriver::addFile new fd " << fd
                  << " for file " << file_name;
    EV_SET(&changelist,
           fd,                                                    // ident
           EVFILT_VNODE,                                          // filter
    ...
    if (kevent(kqueue_fd_, &changelist, 1, NULL, 0, &no_timeout) != -1)
      LOG(logDEBUG)
          << "KQueueWatchTowerDriver::addFile new kevent added for fd " << fd;
    else
      LOG(logERROR) << "KQueueWatchTowerDriver::addFile cannot add file";
  } else {
    LOG(logERROR) << "KQueueWatchTowerDriver::addFile cannot open "
                  << file_name;
  }
...
KQueueWatchTowerDriver::SymlinkId KQueueWatchTowerDriver::addSymlink(
  ...
  LOG(logDEBUG) << "INotifyWatchTower::addFile new inotify symlink_wd " <<
  symlink_wd;
...
KQueueWatchTowerDriver::DirId KQueueWatchTowerDriver::addDir(
  ...
  if (fd != -1) {
    LOG(logDEBUG) << "KQueueWatchTowerDriver::addDir new fd " << fd
                  << " for dir " << file_name;
    EV_SET(&changelist,
           fd,                             // ident
           EVFILT_VNODE,                   // filter
    ...
    // Add a watch for the inode
    if (kevent(kqueue_fd_, &changelist, 1, NULL, 0, &no_timeout) != -1)
      LOG(logDEBUG) << "KQueueWatchTowerDriver::addDir new kevent added for fd "
                    << fd;
    else
...
void KQueueWatchTowerDriver::removeFile(
  ...
  LOG(logDEBUG) << "KQueueWatchTowerDriver::removeFile removing fd "
                << file_id.fd_;
  close(file_id.fd_);
}
...
void KQueueWatchTowerDriver::removeDir(const DirId& dir_id) {
  /*
 LOG(logDEBUG) << "INotifyWatchTower::removeDir removing inotify wd " <<
 dir_id.wd_;
...
KQueueWatchTowerDriver::waitAndProcessEvents(
    KQueueObservedFileList* list, std::unique_lock<std::mutex>* list_lock,
    std::vector<KQueueObservedFile*>* files_needing_readding, int timeout_ms) {
  ...
  list_lock->unlock();
  int event_count =
      kevent(kqueue_fd_, NULL, 0, &event, 1, timeout_ms ? &timeout : NULL);
  list_lock->lock();

  LOG(logDEBUG) << "KQueueWatchTowerDriver::waitAndProcessEvents: event_count "
                << event_count;

  if (event.flags == EV_ERROR) {
    LOG(logERROR) << "kqueue event error!";
  } else if (event_count > 0) {
    LOG(logDEBUG) << "kqueue event received: " << event.ident << " " << std::hex
    ...
    if (file) {
       ...
       * the sledgehammer rather than trying to see what has changed in this
       * dir!
       */
      ...
      files_to_notify.insert(files_to_notify.end(), files_in_dir.begin(),
                             files_in_dir.end());
    }
  } else {
