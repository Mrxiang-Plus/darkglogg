...
INotifyWatchTowerDriver::SymlinkId INotifyWatchTowerDriver::addSymlink(
    const std::string& file_name) {
  int symlink_wd = inotify_add_watch(
      inotify_fd_, file_name.c_str(),
...
void INotifyWatchTowerDriver::removeFile(
  ...
  /*
     ...
     */
  if (file_id.wd_ >= 0) inotify_rm_watch(inotify_fd_, file_id.wd_);
}

void INotifyWatchTowerDriver::removeSymlink(const SymlinkId& symlink_id) {
  if (symlink_id.wd_ >= 0) inotify_rm_watch(inotify_fd_, symlink_id.wd_);
}
...
void INotifyWatchTowerDriver::removeDir(const DirId& dir_id) {
  ...
  if (dir_id.wd_ >= 0) inotify_rm_watch(inotify_fd_, dir_id.wd_);
}
...
INotifyWatchTowerDriver::waitAndProcessEvents(
    INotifyObservedFileList* list, std::unique_lock<std::mutex>* list_lock,
    std::vector<INotifyObservedFile*>* files_needing_readding, int timeout_ms) {
...
size_t INotifyWatchTowerDriver::processINotifyEvent(
    const struct inotify_event* event, INotifyObservedFileList* list,
    std::vector<INotifyObservedFile*>* files_to_notify,
  ...
  if (event->mask & (IN_MODIFY | IN_DELETE_SELF | IN_MOVE_SELF)) {
    LOG(logDEBUG) << "IN_MODIFY | IN_DELETE_SELF | IN_MOVE_SELF for wd "
                  << event->wd;

    // Retrieve the file
    file = list->searchByFileOrSymlinkWd({event->wd}, {event->wd});
  } else if (event->mask & (IN_CREATE | IN_MOVED_TO | IN_MOVED_FROM)) {
    LOG(logDEBUG) << "IN_CREATE | IN_MOVED_TO | IN_MOVED_FROM for wd "
                  << event->wd << " name: " << event->name;
