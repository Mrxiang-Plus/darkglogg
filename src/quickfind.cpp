...
void QuickFind::LastMatchPosition::set(int line, int column) {
  if ((line_ == -1) || ((line <= line_) && (column < column_))) {
    line_ = line;
    column_ = column;
  }
}
...
bool QuickFind::LastMatchPosition::isSooner(
    const FilePosition& position) const {
  return isSooner(position.line(), position.column());
}

QuickFind::QuickFind(const AbstractLogData* const logData, Selection* selection,
  ...
  connect(&searchingNotifier_, SIGNAL(notify(const QFNotification&)), this,
...
qint64 QuickFind::incrementallySearchForward() {
  ...
  if (incrementalSearchStatus_.direction() == Forward) {
    ...
    incrementalSearchStatus_ =
        IncrementalSearchStatus(Forward, start_position, *selection_);
  }
...
qint64 QuickFind::incrementallySearchBackward() {
  ...
  if (incrementalSearchStatus_.direction() == Backward) {
    ...
    // It's a new search so we search from the selection
    incrementalSearchStatus_ =
        IncrementalSearchStatus(Backward, start_position, *selection_);
  }
...
qint64 QuickFind::searchBackward() {
  ...
  LOG(logINFO) << "searchBackward: " << start_position.column();
...
qint64 QuickFind::doSearchForward(const FilePosition& start_position) {
  ...
  if (!quickFindPattern_->isActive()) return -1;
  ...
  if (quickFindPattern_->isLineMatching(logData_->getExpandedLineString(line),
  ...
  if (found) {
    selection_->selectPortion(line, found_start_col, found_end_col);
...
qint64 QuickFind::doSearchBackward(const FilePosition& start_position) {
  ...
  if (firstMatch_.isSooner(start_position)) {
    // Send a notification
    //        selection_->selectPortion( firstMatch_.line_, firstMatch_.column_,
    //        firstMatch_.column_+5);
  ...
  qint64 line = start_position.line();
  LOG(logINFO) << "Start searching at line " << line;
  // We look at the beginning of the first line
  if ((start_position.column() > 0) &&
      (quickFindPattern_->isLineMatchingBackward(
          logData_->getExpandedLineString(line), start_position.column()))) {
    LOG(logINFO) << "do search backward: " << start_col << "end: " << end_col;
    ...
    while (line >= 0) {
      if (quickFindPattern_->isLineMatchingBackward(
              logData_->getExpandedLineString(line))) {
        LOG(logINFO) << "do search backward: " << start_col
                     << "end: " << end_col;
        quickFindPattern_->getLastMatch(&start_col, &end_col);
        found = true;
        break;
  ...
  if (found) {
    ...
    FilePosition first_match_position = selection_->getPreviousPosition();
    firstMatch_.set(first_match_position);

    // Send a notification
    LOG(logINFO) << "QF: Send BOF notification."
                 << first_match_position.column();
    emit notify(QFNotificationReachedBegininningOfFile());
