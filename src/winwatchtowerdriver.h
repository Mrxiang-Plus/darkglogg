...
class WinWatchTowerDriver {
 public:
  struct WinWatchedDirRecord {
    WinWatchedDirRecord(const std::string& file_name) : path_(file_name) {}
  ...
  class DirId {
   public:
    ...
    bool valid() const { return (dir_record_ != nullptr); }
  ...
  class FileChangeToken {
   public:
    FileChangeToken() {}
    FileChangeToken(const std::string& file_name) { readFromFile(file_name); }
    ...
    bool operator!=(const FileChangeToken& other) { return !operator==(other); }
 ...
 private:
  ...
  void serialisedAddDir(const std::string& file_name, DirId& dir_id);
};
