...
class SessionInfo : public Persistable {
 public:
  ...
  QByteArray geometry() const { return geometry_; }
  void setGeometry(const QByteArray& geometry) { geometry_ = geometry; }
  ...
  std::vector<OpenFile> openFiles() const { return openFiles_; }
