...
void Selection::selectPortion(int line, int start_column, int end_column) {
  LOG(logINFO) << "select portion: " << line << " start :" << start_column
               << "end " << end_column;
...
void Selection::crop(int last_line) {
  ...
  if (selectedPartial_.line > last_line) selectedPartial_.line = -1;

  if (selectedRange_.endLine > last_line) selectedRange_.endLine = last_line;

  if (selectedRange_.startLine > last_line)
    selectedRange_.startLine = last_line;
};

bool Selection::getPortionForLine(int line, int* start_column,
...
qint64 Selection::selectedLine() const { return selectedLine_; }
...
QString Selection::getSelectedText(const AbstractLogData* logData) const {
  ...
  if (selectedLine_ >= 0) {
    ...
    QStringList list = logData->getLines(
...
FilePosition Selection::getNextPosition() const {
  ...
  LOG(logINFO) << "next " << line << "line:" << column;
...
FilePosition Selection::getPreviousPosition() const {
  ...
  LOG(logINFO) << "pre " << line << "line:" << column;
  return FilePosition(line, column);
}
