...
class LinePosition {
 public:
  ...
  // Default constructor
  LinePosition() : array() { fakeFinalLF_ = false; }
  ...
  inline void append(uint64_t pos) {
    if (fakeFinalLF_) array.pop_back();
  ...
  // Extract an element
  inline uint64_t at(int i) const { return array.at(i); }
  inline uint64_t operator[](int i) const { return array.at(i); }
  // Set the presence of a fake final LF
  // Must be used after 'append'-ing a fake LF at the end.
  void setFakeFinalLF(bool finalLF = true) { fakeFinalLF_ = finalLF; }
  ...
  void append_list(const LinePosition<SimpleLinePositionStorage>& other) {
    ...
    if (fakeFinalLF_) this->array.pop_back();
