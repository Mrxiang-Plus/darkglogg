...
LogData::LogData()
  ...
  connect(fileWatcher_.get(), SIGNAL(fileChanged(const QString&)), this,
          SLOT(fileChangedOnDisk()));
  // Forward the update signal
  connect(&workerThread_, SIGNAL(indexingProgressed(int)), this,
          SIGNAL(loadingProgressed(int)));
  connect(&workerThread_, SIGNAL(indexingFinished(LoadingStatus)), this,
          SLOT(indexingFinished(LoadingStatus)));
...
LogData::~LogData() {
  ...
  if (attached_file_) fileWatcher_->removeFile(attached_file_->fileName());
...
void LogData::attachFile(const QString& fileName) {
  ...
  std::shared_ptr<const LogDataOperation> operation(
      new AttachOperation(fileName));
  enqueueOperation(std::move(operation));
}

void LogData::interruptLoading() { workerThread_.interrupt(); }

qint64 LogData::getFileSize() const { return indexing_data_.getSize(); }

QDateTime LogData::getLastModifiedDate() const { return lastModifiedDate_; }
...
void LogData::enqueueOperation(
    std::shared_ptr<const LogDataOperation> new_operation) {
...
void LogData::fileChangedOnDisk() {
  ...
  // has been moved and the inode we are following is now under a new name, if
  // for instance log has been rotated). We want to follow the name so we have
  // to reopen the file to ensure we are reading the right one. This is a crude
  // heuristic but necessary for notification services that do not give details
  // (e.g. kqueues)
  if ((info.size() != attached_file_->size()) ||
      (attached_file_->openMode() == QIODevice::NotOpen)) {
    LOG(logINFO)
        << "Inconsistent size, the file might have changed, re-opening";
    // TODO: change to reload
    reOpenFile();
    if (info.size() == attached_file_->size()) {
      emit fileCommented(fileChangedOnDisk_);
    }

    // We don't force a (slow) full reindex as this routinely happens if
    // the file is appended quickly.
    // This means we can occasionally have false negatives (should be dealt with
    // at a lower level): e.g. if a new file is created with the same name as
    // the old one and with a size greater than the old one (should be rare in
    // practice).
  }
...
void LogData::indexingFinished(LoadingStatus status) {
  LOG(logDEBUG) << "indexingFinished: " << (status == LoadingStatus::Successful)
  ...
  if (status == LoadingStatus::Successful) {
    ...
    if (fileInfo.exists()) lastModifiedDate_ = fileInfo.lastModified();
  }
...
//
qint64 LogData::doGetNbLine() const { return indexing_data_.getNbLines(); }

int LogData::doGetMaxLength() const { return indexing_data_.getMaxLength(); }
...
void LogData::doSetDisplayEncoding(Encoding encoding) {
  LOG(logDEBUG) << "AbstractLogData::setDisplayEncoding: "
...
QString LogData::doGetLineString(qint64 line) const {
  ...
  const qint64 first_byte =
      (line == 0) ? 0
                  : (indexing_data_.getPosForLine(line - 1) + after_cr_offset_);
  const qint64 end_byte = endOfLinePosition(line);

  attached_file_->seek(first_byte);

  QString string =
      codec_->toUnicode(attached_file_->read(end_byte - first_byte));
...
QStringList LogData::doGetExpandedLines(qint64 first_line, int number) const {
  ...
  if (last_line >= indexing_data_.getNbLines()) {
    LOG(logWARNING)
        << "LogData::doGetExpandedLines Lines out of bound asked for";
    return QStringList(); /* exception? */
  }
  ...
  const qint64 first_byte =
      (first_line == 0)
          ...
          : (indexing_data_.getPosForLine(first_line - 1) + after_cr_offset_);
  const qint64 end_byte = endOfLinePosition(last_line);
  LOG(logDEBUG) << "LogData::doGetExpandedLines first_byte:" << first_byte
                << " end_byte:" << end_byte;
...
void LogData::reOpenFile() {
  ...
  attached_file_ =
      std::move(reopened);  // This will close the old one and open the new
}
