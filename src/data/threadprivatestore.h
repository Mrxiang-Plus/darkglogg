...
class ThreadPrivateStore {
 public:
  ...
  ThreadPrivateStore() : nb_threads_(0) {
    // Last one is a guard
    for (int i = 0; i < (MAX_THREADS + 1); ++i) thread_ids_[i] = 0;
  }
  ...
  T get() const { return data_[threadIndex()]; }

  T* getPtr() { return &data_[threadIndex()]; }

  // Setter (thread-safe, wait-free)
  void set(const T& value) { data_[threadIndex()] = value; }
  ...
  int threadIndex() const {
    ...
    while (i < MAX_THREADS) {
      ...
      if (thread_ids_[i++].compare_exchange_weak(expected, thread_id)) {
        LOG(logDEBUG) << "Created thread for " << thread_id << " at index "
                      << i - 1;
        return i - 1;
      }
    }
