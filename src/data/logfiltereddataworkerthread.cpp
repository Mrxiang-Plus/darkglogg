...
void SearchData::setAll(int length, SearchResultArray&& matches) {
  QMutexLocker locker(&dataMutex_);
...
void SearchData::addAll(int length, const SearchResultArray& matches,
                        LineNumber lines) {
  QMutexLocker locker(&dataMutex_);
...
void LogFilteredDataWorkerThread::search(const QRegularExpression& regExp) {
  ...
  while ((operationRequested_ != NULL)) nothingToDoCond_.wait(&mutex_);

  interruptRequested_ = false;
  operationRequested_ =
      new FullSearchOperation(sourceLogData_, regExp, &interruptRequested_);
  operationRequestedCond_.wakeAll();
}
...
void LogFilteredDataWorkerThread::updateSearch(const QRegularExpression& regExp,
  ...
  interruptRequested_ = false;
  operationRequested_ = new UpdateSearchOperation(
      sourceLogData_, regExp, &interruptRequested_, position);
  operationRequestedCond_.wakeAll();
}
...
void LogFilteredDataWorkerThread::run() {
  ...
  forever {
    ...
    if (terminate_) return;  // We must die
...
void SearchOperation::doSearch(SearchData& searchData, qint64 initialLine) {
  ...
  LOG(logDEBUG) << "Searching from line " << initialLine << " to "
                << nbSourceLines;

  for (qint64 i = initialLine; i < nbSourceLines; i += nbLinesInChunk) {
    if (*interruptRequested_) break;

    const int percentage =
        (i - initialLine) * 100 / (nbSourceLines - initialLine);
    emit searchProgressed(nbMatches, percentage, initialLine);

    const QStringList lines = sourceLogData_->getLines(
        i, qMin(nbLinesInChunk, (int)(nbSourceLines - i)));
    LOG(logDEBUG) << "Chunk starting at " << i << ", " << lines.size()
                  << " lines read.";
    ...
    for (; j < lines.size(); j++) {
      if (regexp_.match(lines[j]).hasMatch()) {
        ...
        const int length =
            sourceLogData_->getExpandedLineString(i + j).length();
