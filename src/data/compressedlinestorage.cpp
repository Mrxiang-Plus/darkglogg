...
char* block32_new(int block_size, uint32_t initial_position, char** block_ptr) {
...
// initialised at the passed position
char* block64_new(int block_size, uint64_t initial_position, char** block_ptr) {
  // malloc a block of the maximum possible size
  // (where every line is >16384)
...
void CompressedLinePositionStorage::append(uint64_t pos) {
  ...
  if (!block_pointer_) {
    ...
    if (!store_in_big)
      block32_index_.push_back(block32_new(BLOCK_SIZE, pos, &block_pointer_));
    else
      block64_index_.push_back(block64_new(BLOCK_SIZE, pos, &block_pointer_));
  } else {
  ...
  if (!store_in_big) {
    if (nb_lines_ % BLOCK_SIZE == 0) {
      ...
      size_t new_size =
          (previous_block_pointer_ + sizeof(uint16_t) + sizeof(uint32_t)) -
          block;
      ...
      block_pointer_ = nullptr;
      previous_block_pointer_ =
          static_cast<char*>(new_location) + (previous_block_pointer_ - block);
    }
  } else {
    if ((nb_lines_ - first_long_line_) % BLOCK_SIZE == 0) {
      ...
      // is replaced by an absolute value in the future (following a pop_back)
      size_t new_size =
          (previous_block_pointer_ + sizeof(uint16_t) + sizeof(uint64_t)) -
          block;
      void* new_location = realloc(block, new_size);
      if (new_location)
...
void CompressedLinePositionStorage::append_list(
  ...
  // reasonably fast.
  for (uint32_t i = 0; i < positions.size(); i++) append(positions.at(i));
}
