...
void LogDataWorkerThread::indexAll() {
  ...
  interruptRequested_ = false;
  operationRequested_ = new FullIndexOperation(
      fileName_, indexing_data_, &interruptRequested_, &encodingSpeculator_);
  operationRequestedCond_.wakeAll();
}
...
void LogDataWorkerThread::indexAdditionalLines() {
  ...
  interruptRequested_ = false;
  operationRequested_ = new PartialIndexOperation(
      fileName_, indexing_data_, &interruptRequested_, &encodingSpeculator_);
  operationRequestedCond_.wakeAll();
}
...
void LogDataWorkerThread::run() {
  ...
  forever {
    ...
    if (operationRequested_) {
      connect(operationRequested_, SIGNAL(indexingProgressed(int)), this,
              SIGNAL(indexingProgressed(int)));
...
void IndexOperation::doIndex(IndexingData* indexing_data,
                             EncodingSpeculator* encoding_speculator,
                             qint64 initialPosition) {
  qint64 pos =
      initialPosition;        // Absolute position of the start of current line
  ...
  if (file.open(QIODevice::ReadOnly)) {
    ...
    while (!file.atEnd()) {
      ...
      if (*interruptRequest_)  // a bool is always read/written atomically isn't
                               // it?
        break;
          ...
          if (pos_within_block < block.length()) {
            ...
            if (c == '\n')
              ...
              additional_spaces += AbstractLogData::tabStop -
                                   (((block_beginning - pos) +
                                     pos_within_block + additional_spaces) %
