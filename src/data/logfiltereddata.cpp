/*
 * Copyright (C) 2009, 2010, 2011, 2012, 2013, 2017 Nicolas Bonnefon and other
 * contributors
 *
...
LogFilteredData::LogFilteredData(const LogData* logData)
  ...
  connect(&workerThread_, SIGNAL(searchProgressed(int, int, qint64)), this,
          SLOT(handleSearchProgressed(int, int, qint64)));
...
bool LogFilteredData::isLineInMatchingList(qint64 lineNumber) {
  int index;  // Not used
  return lookupLineNumber<SearchResultArray>(matching_lines_, lineNumber,
                                             &index);
}
...
LineNumber LogFilteredData::getNbMarks() const { return marks_.size(); }

LogFilteredData::FilteredLineType LogFilteredData::filteredLineTypeByIndex(
    int index) const {
  ...
  if (visibility_ == MatchesOnly)
    ...
    if (filteredItemsCacheDirty_) regenerateFilteredItemsCache();
...
void LogFilteredData::addMark(qint64 line, QChar mark) {
  if ((line >= 0) && (line < sourceLogData_->getNbLine())) {
    ...
    maxLengthMarks_ =
        qMax(maxLengthMarks_, sourceLogData_->getLineLength(line));
    filteredItemsCacheDirty_ = true;
  } else
...
void LogFilteredData::deleteMark(qint64 line) {
  ...
  if (sourceLogData_->getLineLength(line) >= maxLengthMarks_) {
    ...
    for (Marks::const_iterator i = marks_.begin(); i != marks_.end(); ++i) {
      ...
      maxLengthMarks_ =
          qMax(maxLengthMarks_, sourceLogData_->getLineLength(i->lineNumber()));
    }
  }
}
...
void LogFilteredData::setVisibility(Visibility visi) { visibility_ = visi; }
...
LineNumber LogFilteredData::findFilteredLine(LineNumber lineNum) const {
  ...
  if (visibility_ == MatchesOnly) {
    lineIndex = lookupLineNumber(matching_lines_.begin(), matching_lines_.end(),
                                 lineNum);
  } else if (visibility_ == MarksOnly) {
    ...
    lineIndex = lookupLineNumber(filteredItemsCache_.begin(),
                                 filteredItemsCache_.end(), lineNum);
  }
...
void LogFilteredData::regenerateFilteredItemsCache() const {
  ...
  while ((i != matching_lines_.cend()) || (j != marks_.end())) {
    qint64 next_mark = (j != marks_.end()) ? j->lineNumber()
