...
void OverviewWidget::paintEvent(QPaintEvent* /* paintEvent */) {
  static const QColor match_color(QColor(255, 203, 107));
  // static const QColor mark_color("dodgerblue");
  static const QColor mark_color(QColor(255, 83, 112));

  static const QPixmap highlight_pixmap[] = {
      QPixmap(highlight_xpm[0]), QPixmap(highlight_xpm[1]),
      QPixmap(highlight_xpm[2]), QPixmap(highlight_xpm[3]),
      QPixmap(highlight_xpm[4]), QPixmap(highlight_xpm[5]),
  };
...

  {
    ...
    foreach (Overview::WeightedLine line, *(overview_->getMatchLines())) {
      ...
      painter.drawLine(1 + LINE_MARGIN + 2, line.position(),
                       width() - LINE_MARGIN - 2, line.position());
    }
    ...
    foreach (Overview::WeightedLine line, *(overview_->getMarkLines())) {
      ...
      // (allow multiple matches to look 'darker' than a single one.)
      painter.drawLine(1 + LINE_MARGIN, line.position(),
                       width() - LINE_MARGIN - 1, line.position());
    }
...
void OverviewWidget::handleMousePress(int position) {
  ...
  LOG(logDEBUG) << "OverviewWidget::handleMousePress y=" << position
                << " line=" << line;
