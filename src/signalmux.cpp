...
SignalMux::SignalMux() : connectionList_() { currentDocument_ = nullptr; }

void SignalMux::connect(QObject* sender, const char* signal, const char* slot) {
  Connection new_connection = {sender, nullptr, signal, slot};
...
void SignalMux::disconnect(QObject* sender, const char* signal,
                           const char* slot) {
...
// Upstream signals
void SignalMux::connect(const char* signal, QObject* receiver,
                        const char* slot) {
  Connection new_connection = {nullptr, receiver, signal, slot};
...
void SignalMux::disconnect(const char* signal, QObject* receiver,
                           const char* slot) {
  // Find any signal that match our description
  auto connection = std::find_if(
      connectionList_.begin(), connectionList_.end(),
...
void SignalMux::setCurrentDocument(QObject* current_document) {
  ...
  for (auto c : connectionList_) disconnect(c);

  currentDocument_ = current_document;

  // And now connect to/from the new document
  for (auto c : connectionList_) connect(c);
  ...
  if (doc) doc->sendAllStateSignals();
}
...
void SignalMux::connect(const Connection& connection) {
  if (currentDocument_) {
    ...
    if (connection.source && (!connection.sink)) {
      // Downstream signal
      QObject::connect(connection.source, connection.signal, currentDocument_,
                       connection.slot);
    } else if ((!connection.source) && connection.sink) {
      // Upstream signal
      QObject::connect(currentDocument_, connection.signal, connection.sink,
                       connection.slot);
    } else {
...
void SignalMux::disconnect(const Connection& connection) {
  if (currentDocument_) {
    ...
    if (connection.source && (!connection.sink)) {
      ...
      // Upstream signal
      QObject::disconnect(currentDocument_, connection.signal, connection.sink,
                          connection.slot);
    } else {
      LOG(logERROR) << "SignalMux has an unexpected signal";
    }
