...
const char* WinNotificationInfoList::updateCurrentNotification(
  ...
  const std::wstring file_name = {
      reinterpret_cast<const wchar_t*>(new_position + 12), length / 2};
  ...
  current_notification_ =
      WinNotificationInfo(int_to_action.at(action), file_name);
...
const char* WinNotificationInfoList::advanceToNext() {
  pointer_ = next_;
  if (pointer_) next_ = updateCurrentNotification(pointer_);
...
void WinWatchTowerDriver::FileChangeToken::readFromFile(
  ...
  if (hFile == (HANDLE)-1) {
    ...
    if (GetFileInformationByHandle(hFile, &file_info)) {
      ...
      LOG(logDEBUG) << "FileChangeToken::readFromFile: low_date_time_ "
                    << low_date_time_;
      LOG(logDEBUG) << "FileChangeToken::readFromFile: high_date_time_ "
                    << high_date_time_;
    } else {
      DWORD err = GetLastError();
      LOG(logERROR) << "FileChangeToken::readFromFile: failed with " << err;
...
WinWatchTowerDriver::WinWatchTowerDriver() {
  hCompPort_ = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0x0, 0);
}
...
void WinWatchTowerDriver::removeFile(const WinWatchTowerDriver::FileId&) {}
...
void WinWatchTowerDriver::removeDir(const DirId& dir_id) {
  ...
  if (dir_id.dir_record_) {
    ...
    LOG(logDEBUG) << "WinWatchTowerDriver::removeDir handle=" << std::hex
                  << handle;
...
void WinWatchTowerDriver::serialisedAddDir(const std::string& dir_name,
                                           DirId& dir_id) {
  bool inserted = false;
  ...
  if (hDir == INVALID_HANDLE_VALUE) {
    ...
    hCompPort_ = CreateIoCompletionPort(
        hDir,
        hCompPort_,  // if m_hCompPort is NULL, hDir is associated with a NEW
                     // completion port,
        // if m_hCompPort is NON-NULL, hDir is associated with the existing
        // completion port that the handle m_hCompPort references
    ...
    inserted = ReadDirectoryChangesW(
        hDir, dir_record->buffer_, dir_record->buffer_length_, false,
...
std::vector<ObservedFile<WinWatchTowerDriver>*>
WinWatchTowerDriver::waitAndProcessEvents(
    ObservedFileList<WinWatchTowerDriver>* list,
    std::unique_lock<std::mutex>* lock,
    std::vector<ObservedFile<
        WinWatchTowerDriver>*>* /* not needed in WinWatchTowerDriver */,
    int timeout_ms) {
  ...
  BOOL status = GetQueuedCompletionStatus(hCompPort_, &num_bytes, &key,
  ...
  if (key) {
    ...
    if (std::shared_ptr<WinWatchedDirRecord> dir_record =
            dir_record_ptr.lock()) {
      ...
      for (auto notification : notification_info) {
        std::string file_path =
            dir_record->path_ + shortstringize(notification.fileName());
      ...
      status = ReadDirectoryChangesW(
          dir_record->handle_, dir_record->buffer_, dir_record->buffer_length_,
