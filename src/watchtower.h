...
void WatchTower<Driver>::setPollingInterval(uint32_t interval_ms) {
  if (polling_interval_ms_ != interval_ms) {
    ...
    if (polling_interval_ms_ > 0) driver_.interruptWait();
  }
}

template <typename Driver>
Registration WatchTower<Driver>::addFile(const std::string& file_name,
                                         std::function<void()> notification) {
  ...
  auto existing_observed_file = observed_file_list_.searchByName(file_name);

  std::shared_ptr<std::function<void()>> ptr(
      new std::function<void()>(std::move(notification)));
  ...
  if (!existing_observed_file) {
    ...
    if (!dir) {
      ...
      dir = observed_file_list_.addWatchedDirectoryForFile(
          file_name, [this, weakHeartBeat](ObservedDir<Driver>* dir) {
      ...
      LOG(logDEBUG) << "WatchTower::addFile Found exisiting watch for dir "
                    << file_name;
    }

    // Associate the dir to the file
    if (dir) new_file->dir_ = dir;
  } else {
    LOG(logDEBUG)
        << "WatchTower::addFile add extra callback for already monitored "
        << file_name;
...
void WatchTower<Driver>::removeNotification(
  ...
  auto file = watch_tower->observed_file_list_.removeCallback(notification);
...
void WatchTower<Driver>::run() {
  while (running_) {
    ...
    auto files = driver_.waitAndProcessEvents(&observed_file_list_, &lock,
                                              &files_needing_readding,
      ...
      for (auto file : observed_file_list_) {
        uint32_t ms_since_last_check =
            std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now() - file->timeForLastCheck())
                .count();
        if ((ms_since_last_check > polling_interval_ms_) &&
            file->hasChanged()) {
          LOG(logDEBUG) << "WatchTower::run: " << file->file_name_;
          for (auto observer : file->callbacks) {
            LOG(logDEBUG) << "WatchTower::run: notifying the client because of "
                             "a timeout!";
            // Here we have to cast our generic pointer back to
            // the function pointer in order to perform the call
            const std::shared_ptr<std::function<void()>> fptr =
