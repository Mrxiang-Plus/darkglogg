 ...
 public:
  class FileId {
   public:
    ...
    FileId() { wd_ = -1; }
    bool operator==(const FileId& other) const { return wd_ == other.wd_; }
  ...
  class DirId {
   public:
    ...
    DirId() { wd_ = -1; }
    bool operator==(const DirId& other) const { return wd_ == other.wd_; }
    bool valid() const { return (wd_ != -1); }
  ...
  class SymlinkId {
   public:
    ...
    SymlinkId() { wd_ = -1; }
    bool operator==(const SymlinkId& other) const { return wd_ == other.wd_; }
  ...
  class FileChangeToken {
   public:
    ...
    bool operator!=(const FileChangeToken&) { return true; }
  };
  ...
  std::vector<INotifyObservedFile*> waitAndProcessEvents(
      INotifyObservedFileList* list, std::unique_lock<std::mutex>* list_mutex,
