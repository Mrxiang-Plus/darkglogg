...
static const char* ml_format =
    "LOGDATA is a part of glogg, we are going to test it thoroughly, this is "
    "line\t\t%06d\n";
static const int ML_VISIBLE_LINE_LENGTH =
    (76 + 8 + 4 + 6);  // Without the final '\n' !
...
TestLogFilteredData::TestLogFilteredData()
    : QObject(),
...
void TestLogFilteredData::initTestCase() { QVERIFY(generateDataFiles()); }

void TestLogFilteredData::simpleSearch() {
  logData_ = new LogData();

  // Register for notification file is loaded
  connect(logData_, SIGNAL(loadingFinished(bool)), this,
          SLOT(loadingFinished()));

  filteredData_ = logData_->getNewFilteredData();
  connect(filteredData_, SIGNAL(searchProgressed(int, int)), this,
          SLOT(searchProgressed(int, int)));

  QFuture<void> future =
      QtConcurrent::run(this, &TestLogFilteredData::simpleSearchTest);
...
void TestLogFilteredData::multipleSearch() {
  ...
  QFuture<void> future =
      QtConcurrent::run(this, &TestLogFilteredData::multipleSearchTest);
...
void TestLogFilteredData::updateSearch() {
  ...
  QFuture<void> future =
      QtConcurrent::run(this, &TestLogFilteredData::updateSearchTest);
...
void TestLogFilteredData::updateSearchTest() {
  ...
  if (file.open(QIODevice::Append)) {
    ...
    // To test the edge case when the final line is not complete and not
    // matching
...
void TestLogFilteredData::marks() {
  ...
  QFuture<void> future =
      QtConcurrent::run(this, &TestLogFilteredData::marksTest);
...
void TestLogFilteredData::marksTest() {
  ...
  // Performs a search
  QSignalSpy progressSpy(filteredData_, SIGNAL(searchProgressed(int, int)));
...
void TestLogFilteredData::lineLength() {
  ...
  QFuture<void> future =
      QtConcurrent::run(this, &TestLogFilteredData::lineLengthTest);
...
void TestLogFilteredData::loadingFinished() {
  ...
  while (!loadingFinished_read_) loadingFinishedCondition_.wait(locker.mutex());
}
...
bool TestLogFilteredData::generateDataFiles() {
  ...
  if (file.open(QIODevice::WriteOnly)) {
    ...
    file.write(
        "This line is very long, it's actually hard to count but it is\
