...
TEST_F(LogDataChanging, changingFile) {
  ...
  {
    SafeQSignalSpy finishedSpy(&log_data,
...
class LogDataBehaviour : public testing::Test {
 public:
  LogDataBehaviour() { generateDataFiles(); }
...
class LogDataMultiByte : public testing::Test {
 public:
  LogDataMultiByte() { generateDataFiles(); }
...
TEST_F(LogDataMultiByte, readUtf16LE) {
  ...
  ASSERT_THAT(log_data.getDetectedEncoding(),
              EncodingSpeculator::Encoding::UTF16LE);
  ASSERT_THAT(
      QString::compare(log_data.getLineString(3), QStringLiteral("1665")), 0);
  ASSERT_THAT(QString::compare(log_data.getExpandedLineString(4),
                               QStringLiteral("Molière")),
              0);
  ASSERT_THAT(QString::compare(log_data.getLines(11, 3).at(0),
                               QStringLiteral("ELVIRE, femme de Dom Juan.")),
              0);
  ASSERT_THAT(QString::compare(log_data.getLines(11, 3).at(1),
                               QStringLiteral("GUSMAN, écuyer d'Elvire.")),
              0);
  ASSERT_THAT(QString::compare(log_data.getLines(11, 3).at(2),
                               QStringLiteral("DOM CARLOS, frère d'Elvire.")),
              0);
  ASSERT_THAT(QString::compare(log_data.getExpandedLines(0, 3).at(2),
                               QStringLiteral("COMÉDIE")),
              0);
}
...
TEST_F(LogDataMultiByte, readUtf16BE) {
  ...
  ASSERT_THAT(log_data.getDetectedEncoding(),
              EncodingSpeculator::Encoding::UTF16BE);
  ASSERT_THAT(
      QString::compare(log_data.getLineString(3), QStringLiteral("1665")), 0);
  ASSERT_THAT(QString::compare(log_data.getExpandedLineString(4),
                               QStringLiteral("Molière")),
