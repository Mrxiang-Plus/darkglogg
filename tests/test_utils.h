...
struct TestTimer {
  TestTimer()
      : TestTimer(::testing::UnitTest::GetInstance()
                      ->current_test_info()
                      ->test_case_name()) {
    text_ +=
        std::string{"."} +
  ...
  TestTimer(const std::string& text)
      : Start{std::chrono::system_clock::now()}, text_{text} {}
  ...
  virtual ~TestTimer() {
    ...
    cout << endl
         << text_ << " elapsed time = " << Elapsed.count() * 0.001 << "ms"
         << endl;
  }
...
class SafeQSignalSpy : public QSignalSpy {
 public:
  ...
  SafeQSignalSpy(Args&&... args) : QSignalSpy(std::forward<Args>(args)...) {}
