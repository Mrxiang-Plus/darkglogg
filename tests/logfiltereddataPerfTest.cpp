...
class PerfLogFilteredData : public testing::Test {
 public:
  PerfLogFilteredData()
      : log_data_(),
        filtered_data_(log_data_.getNewFilteredData()),
    ...
    if (!endSpy.wait(999000)) std::cerr << "Unable to attach the file!";
  }
...
TEST_F(PerfLogFilteredData, browsingSearchResults) {
  ...
  QStringList list;
  {
    ...
    for (int page = 0; page < (nb_results / VBL_LINE_PER_PAGE) - 1; page++) {
      list = filtered_data_->getExpandedLines(page * VBL_LINE_PER_PAGE,
                                              VBL_LINE_PER_PAGE);
      ASSERT_THAT(list.count(), VBL_LINE_PER_PAGE);
      int page_from_end = (nb_results / VBL_LINE_PER_PAGE) - page - 1;
      list = filtered_data_->getExpandedLines(page_from_end * VBL_LINE_PER_PAGE,
                                              VBL_LINE_PER_PAGE);
      ASSERT_THAT(list.count(), VBL_LINE_PER_PAGE);
    }
  }
