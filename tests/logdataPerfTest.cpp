...
TEST_F(PerfLogData, simpleLoad) {
  ...
  ASSERT_THAT(progressSpy.count(),
              log_data.getFileSize() / (5LL * 1024 * 1024) + 2);
...
class PerfLogDataRead : public PerfLogData {
 public:
  PerfLogDataRead()
      : PerfLogData(),
        log_data(),
        endSpy(&log_data, SIGNAL(loadingFinished(LoadingStatus))) {
    log_data.attachFile(TMPDIR "/verybiglog.txt");
    endSpy.wait(20000);
  }
...
TEST_F(PerfLogDataRead, randomPageRead) {
  ...
  QStringList list;
  {
    ...
    for (int page = 0; page < (VBL_NB_LINES / VBL_LINE_PER_PAGE) - 1; page++) {
      ...
      list = log_data.getLines(page_from_end * VBL_LINE_PER_PAGE,
                               VBL_LINE_PER_PAGE);
      ASSERT_THAT(list.count(), VBL_LINE_PER_PAGE);
    }
  }
...
TEST_F(PerfLogDataRead, randomPageReadExpanded) {
  ...
  QStringList list;
  {
    ...
    for (int page = 0; page < (VBL_NB_LINES / VBL_LINE_PER_PAGE) - 1; page++) {
      list = log_data.getExpandedLines(page * VBL_LINE_PER_PAGE,
                                       VBL_LINE_PER_PAGE);
      ASSERT_THAT(list.count(), VBL_LINE_PER_PAGE);
      int page_from_end = (VBL_NB_LINES / VBL_LINE_PER_PAGE) - page - 1;
      list = log_data.getExpandedLines(page_from_end * VBL_LINE_PER_PAGE,
                                       VBL_LINE_PER_PAGE);
      ASSERT_THAT(list.count(), VBL_LINE_PER_PAGE);
    }
  }
