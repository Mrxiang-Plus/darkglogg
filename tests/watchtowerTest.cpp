...
class WatchTowerBehaviour : public testing::Test {
 public:
  shared_ptr<PlatformWatchTower> watch_tower =
      make_shared<PlatformWatchTower>();
...
TEST_F(WatchTowerBehaviour, AcceptsANonExistingFileToWatch) {
  auto registration =
      watch_tower->addFile(getNonExistingFileName(), [](void) {});
}
...
class WatchTowerSingleFile : public WatchTowerBehaviour {
 public:
  ...
  ~WatchTowerSingleFile() { remove(file_name.c_str()); }
...
class WatchTowerDirectories : public WatchTowerSingleFile {
 public:
  ...
  string createTempDir() {
    ...
    string dir_name =
        string{temp_dir} + string{"\\test"} + to_string(counter++);
  ...
  string createTempEmptyFileInDir(const string& dir) {
    static int counter = 1;
    return createTempEmptyFile(dir + std::string{"/temp"} +
                               to_string(counter++));
  }

  void removeDir(const string& name) { rmdir(name.c_str()); }
};
...
class WatchTowerInexistantDirectory : public WatchTowerDirectories {
 public:
  ...
  ~WatchTowerInexistantDirectory() { rmdir(test_dir.c_str()); }
...
struct WinNotificationInfoListTest::Buffer
    WinNotificationInfoListTest::buffer[] = {{40, 1, 26, L"Filename.txt"},
                                             {0, 3, 18, L"file2.txt"}};
